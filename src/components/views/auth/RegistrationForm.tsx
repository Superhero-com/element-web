...
import PassphraseConfirmField from "matrix-react-sdk/src/components/views/auth/PassphraseConfirmField";
import PassphraseField from "matrix-react-sdk/src/components/views/auth/PassphraseField";
import RegistrationEmailPromptDialog from "matrix-react-sdk/src/components/views/dialogs/RegistrationEmailPromptDialog";
import Field from "matrix-react-sdk/src/components/views/elements/Field";
import withValidation, {
    IValidationResult,
    IFieldState,
} from "matrix-react-sdk/src/components/views/elements/Validation";
import { looksValid as phoneNumberLooksValid, PhoneNumberCountryDefinition } from "matrix-react-sdk/src/phonenumber";
...
export default class RegistrationForm extends React.PureComponent<IProps, IState> {
    ...
    private [RegistrationField.InviteCode]: Field | null = null;
    ...
    public constructor(props: IProps) {
        ...
        this.state = {
            ...
            inviteCode: this.props.defaultInviteCode || "",
        };
    }
        ...
        if (promise) {
            ...
            promise
                .then(() => {
                    ...
                    const username = `@${this.state.username.trim()}:${this.props.serverConfig.hsName}`;
                    const affiliationEndpoint = (SdkConfig.get() as any).affiliation_registration_endpoint;

                    fetch(`${affiliationEndpoint}${username}/${this.state.inviteCode}`, requestOptions)
                        // TODO Remove
    ...
    private validateEmailRules = withValidation({
        ...
        rules: [
            {
                key: "required",
                test(this: RegistrationForm, { value, allowEmpty }): boolean {
                    return allowEmpty || !this.authStepIsRequired("m.login.email.identity") || !!value;
                },
                invalid: (): string => _t("auth|reset_password_email_field_required_invalid"),
            },
            {
                key: "email",
                test: ({ value }): boolean => !value || Email.looksValid(value),
                invalid: (): string => _t("auth|email_field_label_invalid"),
            },
        ],
    });
        ...
        rules: [
            {
                ...
                invalid: (): string => _t("auth|registration_msisdn_field_required_invalid"),
            },
            {
                key: "email",
                test: ({ value }): boolean => !value || phoneNumberLooksValid(value),
    ...
    private onInviteCodeChange = (ev: React.ChangeEvent<HTMLInputElement>): void => {
        this.setState({
            inviteCode: ev.target.value,
        });
    };
    ...
    private onInviteCodeValidate = async (fieldState: IFieldState): Promise<IValidationResult> => {
        const result = await this.validateInviteCodeRules(fieldState);
        this.markFieldValid(RegistrationField.InviteCode, !!result.valid);
        return result;
    };

    private validateInviteCodeRules = withValidation({
        description: () => {
        ...
        rules: [
            {
                ...
                invalid: (): string => _t("auth|username_field_required_invalid"),
            },
            {
                key: "safeLocalpart",
                test: ({ value }, usernameAvailable): boolean =>
                    (!value || SAFE_LOCALPART_REGEX.test(value)) &&
                    usernameAvailable !== UsernameAvailableStatus.Invalid,
                invalid: (): string => _t("room_settings|general|alias_field_safe_localpart_invalid"),
            },
            {
                key: "available",
                final: true,
                ...
                invalid: (usernameAvailable): string =>
    ...
    private renderEmail(): ReactNode {
        ...
        return (
            <EmailField
                ...
                label={emailLabel as EmailField["props"]["label"]}
    ...
    private renderIviteCode(): ReactNode {
        return (
            <Field
                id="mx_RegistrationForm_inviteCode"
                ref={(field) => (this[RegistrationField.InviteCode] = field)}
                type="text"
                label={_t("invite_code_label")}
                placeholder={_t("invite_code_label")}
                value={this.state.inviteCode}
                onChange={this.onInviteCodeChange}
                onValidate={this.onInviteCodeValidate}
            />
        );
    }
        ...
        return (
            <div>
                <form onSubmit={this.onSubmit}>
                    ...
                    <div className="mx_AuthBody_fieldRow">{this.renderIviteCode()}</div>
                    {emailHelperText}
                    {registerButton}
                </form>
            </div>
